...
var (
	...
	unboundMetrics = []*unboundMetric{
		...
		newUnboundMetric(
			"ip_ratelimited",
			"Total number of rate limited queries.",
			prometheus.CounterValue,
			[]string{"thread"},
		...
		newUnboundMetric(
			"dnscrypt_query",
			"Total number of dnscrypt queries",
			prometheus.CounterValue,
			[]string{"thread", "type"},
			"^thread(\\d+)\\.num\\.dnscrypt\\.(\\w+)$"),
		...
		newUnboundMetric(
			...
			"Total number of TLS session resumptions, these are queries over TLS towards the Unbound server.",
			prometheus.CounterValue,
			nil,
			"^num\\.query\\.resume$"),
		newUnboundMetric(
		...
		newUnboundMetric(
			"query_dnscrypt_shared_secret_cache_miss_total",
